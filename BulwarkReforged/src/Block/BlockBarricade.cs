        ...
        //===============================
            ...
            public override void OnBlockExploded(
                ...
                if (block is BlockBomb) {
                    FortificationModSystem fortSystem = world.Api.ModLoader.GetModSystem<FortificationModSystem>();
                    if (fortSystem.TryGetStronghold(pos, out Stronghold stronghold)) {
                    stronghold.IncreaseSiegeIntensity(1f);
                }
            }
                if (this.Variant["state"] switch {
                    "built"   => "damaged",
