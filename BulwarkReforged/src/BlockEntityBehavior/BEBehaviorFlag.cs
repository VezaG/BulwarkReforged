        ...
        //===============================
            ...
            public override void Initialize(ICoreAPI api, JsonObject properties) {
                ...
                if (this.Banner != null && this.Api is ICoreClientAPI client) {
                    ...
                    client.Tesselator.TesselateShape(this.Banner.Item, Shape.TryGet(client, "bulwarkreforged:shapes/flag/banner.json"), out MeshData meshData);
        ...
        //===============================
                                ...
                                if (itemSlot.Itemstack is ItemStack itemStack)
                                    if (itemStack.Collectible?.NutritionProps is FoodNutritionProperties foodNutrition)
                                        this.cellarExpectancy += foodNutrition.Satiety
                                            * itemStack.StackSize
                                            * (BulwarkReforgedModSystem.ClaimDurationPerSatiety * (1f + this.BlockBehavior.ExpectancyBonus));
            ...
            private void UpdateCellar(float deltaTime) {
                if (this.Stronghold.IsClaimed) {
                    ...
                    float nowDurationPerSatiety = BulwarkReforgedModSystem.ClaimDurationPerSatiety * (1f + this.BlockBehavior.ExpectancyBonus);
            ...
            private void CaptureUpdate(float deltaTime) {
                ...
                if (this.CapturedPercent == 1f) {
                    if (!this.Stronghold.IsClaimed)
                        if (this.Api.World.BlockAccessor.GetTerrainMapheightAt(this.Pos) - this.Pos.Y <= BulwarkReforgedModSystem.UndergroundClaimLimit) {
